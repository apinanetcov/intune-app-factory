...
if ($appJson.InstallerType -eq "MSI") {
    ...
    Write-Host "MSI install command:"
...
elseif ($appJson.InstallerType -eq "EXE") {

    if ([string]::IsNullOrWhiteSpace($appJson.InstallCommand)) {
        throw "InstallerType 'EXE' requires InstallCommand in the app definition."
    }
    ...
    # Replace token
    $installCommand = $appJson.InstallCommand.Replace(
        "%InstallerPath%",
    ...
    throw "InstallerType '$($appJson.InstallerType)' is not supported."
}
...
if ($proc.ExitCode -notin 0,3010) {
    ...
    if ($appJson.InstallerType -eq "MSI" -and (Test-Path $logFile)) {
